...
lu_device_bootloader_nordic_get_hw_platform_id (LuDevice *device, GError **error)
{
	...
	return g_strndup (req->data, req->len);
}
...
lu_device_bootloader_nordic_get_fw_version (LuDevice *device, GError **error)
{
	...
	/* RRRxx.yy_Bzzzz
	 * 012345678901234*/
	micro = (guint16) lu_buffer_read_uint8 (req->data + 10) << 8;
	micro += lu_buffer_read_uint8 (req->data + 12);
	return lu_format_version (lu_buffer_read_uint8 (req->data + 3),
				  lu_buffer_read_uint8 (req->data + 6),
				  micro);
}
